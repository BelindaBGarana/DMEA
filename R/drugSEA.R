drugSEA <- function(data, gmt = NULL, drug = "Drug",
  ...
  GSEA_custom <- function(input.df, gmt.list,
    ...
    GSEA.EnrichmentScore2 <- function(gene.list, gene.set,
      ...
      tag.diff.vector[2:Nh] <-
        tag.loc.vector[2:Nh] - tag.loc.vector[seq_len(Nh - 1)] - 1
    ...
    KSRandomArray <- foreach::foreach(L = seq_len(nperm),
      ...
      for (i in seq_len(length(Drug.Sets.All))) {
        ...
        temp.KSRandomArray[, i] <- GSEA.EnrichmentScore2(gene.list,
                                                         pos_gene_set,
          weighted.score.type = score.weight,
          correl.vector = rank_metric
        )
    ...
    KSRandomArray <- data.frame(matrix(unlist(KSRandomArray),
  ...
  testthat::expect_is(EA.Results, "data.frame")
  ...
  if (nrow(significant.hits) > 0) {
    ...
    volc <- ggplot2::ggplot(data = plot.data,
      ...
      ggplot2::theme(
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 1),
        axis.line = element_line(colour = "black", linewidth = 0.65),
        axis.text = element_text(size = 10), axis.title =
  ...
  return(list(
    gmt = gmt, result = EA.Results, mtn.plots = temp.plot,
