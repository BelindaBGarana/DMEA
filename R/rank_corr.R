rank_corr <- function(data, variable="Drug", value="AUC",type="pearson", min.per.corr=3, plots=TRUE, FDR=0.05, xlab=colnames(data)[2], ylab=value, position.x="mid", position.y="max", se=TRUE){
  ...
  corr.no.na$Pearson.q <- qvalue::qvalue(p=corr.no.na$Pearson.p,pi0=1)$qvalues
  corr.no.na$Spearman.q <- qvalue::qvalue(p=corr.no.na$Spearman.p,pi0=1)$qvalues
